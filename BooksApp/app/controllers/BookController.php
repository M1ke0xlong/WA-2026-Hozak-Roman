...
class BookController
{
    ...
    public function create() {
        ...
        // Načtení databáze a modelů
        require_once '../app/models/Database.php';
        require_once '../app/models/Category.php';
        require_once '../app/models/Subcategory.php'; // PŘIDÁNO: Načtení modelu podkategorií
        ...
        // PŘIDÁNO: Získání seznamu podkategorií
        $subcategoryModel = new Subcategory($db);
        $subcategories = $subcategoryModel->getAllSubcategories(); 

        // V šabloně book_create.php nyní budeme mít k dispozici pole $categories i $subcategories
    ...
    public function store() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $subcategory = (int)($_POST['category'] ?? 0);
    ...
    // 6. Zobrazení detailu konkrétní knihy
    public function show($id = null)
    {
        // 1. Zabezpečení: Máme ID? Pokud ne, pošleme uživatele domů
        if (!$id) {
        ...
        // 2. Připojení k databázi a modelu
        require_once '../app/models/Database.php';
        require_once '../app/models/Book.php';
        ...
        // 3. Získání dat z modelu
        $bookModel = new Book($db);
        $book = $bookModel->getById($id); // Tady se volá ta metoda, co už v modelu máš

        // 4. Pokud kniha v DB není (třeba někdo ručně přepsal ID v URL na nesmysl)
        if (!$book) {
            $this->addErrorMessage('Kniha s tímto ID neexistuje.');
            header('Location: ' . BASE_URL . '/index.php');
            exit;
        }

        // 5. Načtení šablony detailu (View)
        // Proměnná $book bude v souboru book_detail.php automaticky dostupná
        require_once '../app/views/books/book_detail.php';
    }
    protected function addSuccessMessage($message)
{
    // Pokud pole pro zprávy ještě neexistuje, vytvoříme ho
    if (!isset($_SESSION['flash_messages'])) {
        $_SESSION['flash_messages'] = [];
    }
    // Přidáme zprávu s typem 'success'
    $_SESSION['flash_messages'][] = [
        'type' => 'success',
        'text' => $message
    ];
