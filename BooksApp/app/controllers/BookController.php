...
class BookController
{
    ...
    {
        // 1. Kontrola, zda bylo předáno ID
        if (!$id) {
        ...
        // 3. Získání dat o konkrétní knize z databáze
        $bookModel = new Book($db);
        $book = $bookModel->getById($id); // Získá data a uloží je do proměnné $book

        // 4. Kontrola, zda kniha existuje
        if (!$book) {
            $this->addErrorMessage('Hledaná kniha nebyla v databázi nalezena.');
            header('Location: ' . BASE_URL . '/index.php');
            exit;
        }

        // 5. Načtení pohledu (View) pro detail knihy
        // Šablona bude mít automaticky přístup k proměnné $book
